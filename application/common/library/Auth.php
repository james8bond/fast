...
class Auth
{
    ...
    /**
     *
     * 通过 openid 注册临时用户, 该账户系统只适用于 小程序使用, 不能在用 username, email, mobile 这三个登录了
     * @param string $openid   微信小程序用户唯一标识
     * @param string $username 用户名 写死 bourne
     * @param string $password 密码  写死 111111
     * @param string $email    邮箱  写死 15974708423@139.com
     * @param string $mobile   手机号写死 15974708423
     * @param array  $extend   扩展参数
     * @return boolean
     */
    public function register_code($openid = '', $username = 'Bourne', $password = '111111', $email = '', $mobile = '', $extend = [])
    {
        // 检测用户名、昵称、邮箱、手机号是否存在
        if (User::getByOpenid($openid)) {
            $this->setError('Openid already exist');
            return false;
        }
//        if (User::getByUsername($username)) {
//            $this->setError('Username already exist');
